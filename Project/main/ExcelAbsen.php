...
header("Content-Disposition: attachment; filename=Absensi_Yantek.xls");
?>
<!DOCTYPE html>
<html>
...
<body>
    ...
    <table>
        ...
        <?php
        include "Connection.php";
        $query = "SELECT * FROM tb_users INNER JOIN tb_absen ON tb_users.id_user = tb_absen.id_user WHERE keterangan = 'Sudah Validasi'";
        if ($startDate && $endDate) {
            $startDate = mysqli_real_escape_string($Conn, $startDate);
            $endDate = mysqli_real_escape_string($Conn, $endDate);
            $query .= " AND tb_absen.tanggal BETWEEN '$startDate' AND '$endDate'";
        }
        $query .= " ORDER BY tb_absen.tanggal DESC";
        $no = 1;
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            <tr>
                ...
                <td>
                    <?php echo date("H.i A", strtotime($row['jam'])); ?>
                </td>
